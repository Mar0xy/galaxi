...
pub fn init_database() -> Result<()> {
    ...
    let conn = Connection::open(&db_path)
        .map_err(|e| GalaxiError::ConfigError(e.to_string()))?;
...
pub fn get_config_value(key: &str) -> Result<String> {
    ...
    let conn = guard.as_ref()
        .ok_or_else(|| GalaxiError::ConfigError("Database not initialized".to_string()))?;
...
pub mod accounts_db {
    ...
    pub fn get_all_accounts() -> Result<Vec<Account>> {
        ...
        for account in accounts {
            result.push(account.map_err(|e| GalaxiError::ConfigError(e.to_string()))?);
        }
    ...
    pub fn get_active_account() -> Result<Option<Account>> {
        ...
        match result {
            ...
            Err(e) => Err(GalaxiError::ConfigError(e.to_string())),
        }
    }
...
pub mod games_db {
    ...
    pub fn get_all_games() -> Result<Vec<Game>> {
        ...
        let mut result = Vec::new();
        for game in games {
            result.push(game.map_err(|e| GalaxiError::ConfigError(e.to_string()))?);
        }
