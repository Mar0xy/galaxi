...
class Config {
    ...
    static loadFromDb() {
        ...
        try {
            ...
            try {
                config.locale = getConfigValue('locale');
            }
            catch (e) { }
            try {
                config.lang = getConfigValue('lang');
            }
            catch (e) { }
            try {
                config.view = getConfigValue('view');
            }
            catch (e) { }
            try {
                config.install_dir = getConfigValue('install_dir');
            }
            catch (e) { }
            try {
                config.username = getConfigValue('username');
            }
            catch (e) { }
            try {
                config.refresh_token = getConfigValue('refresh_token');
            }
            catch (e) { }
            try {
                config.keep_installers = getConfigValue('keep_installers') === 'true';
            }
            catch (e) { }
            try {
                config.stay_logged_in = getConfigValue('stay_logged_in') === 'true';
            }
            catch (e) { }
            try {
                config.use_dark_theme = getConfigValue('use_dark_theme') === 'true';
            }
            catch (e) { }
            try {
                config.show_hidden_games = getConfigValue('show_hidden_games') === 'true';
            }
            catch (e) { }
            try {
                config.show_windows_games = getConfigValue('show_windows_games') === 'true';
            }
            catch (e) { }
            try {
            ...
            // Wine settings
            try {
                config.wine_prefix = getConfigValue('wine_prefix');
            }
            catch (e) { }
            try {
                config.wine_executable = getConfigValue('wine_executable');
            }
            catch (e) { }
            try {
                config.wine_debug = getConfigValue('wine_debug') === 'true';
            }
            catch (e) { }
            try {
                config.wine_disable_ntsync = getConfigValue('wine_disable_ntsync') === 'true';
            }
            catch (e) { }
            try {
                config.wine_auto_install_dxvk = getConfigValue('wine_auto_install_dxvk') !== 'false';
            }
            catch (e) { }
        }
        catch (e) {
